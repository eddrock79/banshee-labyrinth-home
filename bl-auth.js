/* ============================================================
   BANSHEE LABYRINTH — SHARED AUTH  (bl-auth.js)
   ------------------------------------------------------------
   ONE login for every app. Because all the apps live on the same
   origin (eddrock79.github.io), localStorage is shared between them,
   ...
   HOW EACH APP USES IT  (see integration notes):
     <script src="https://eddrock79.github.io/banshee-labyrinth-home/bl-auth.js"></script>
   ...
   ============================================================ */
(function () {
  const PIN_SHEET_ID = '1n_k9VpZmqWhFuj95k0JJFewh8Vi1maYJscPASE09zwg';
  ...
  async function fetchPins(){
    const url = `https://docs.google.com/spreadsheets/d/${PIN_SHEET_ID}/gviz/tq?tqx=out:csv&sheet=Settings`;
    ...
    try {
      ...
      let staff = '', manager = '';
      if (lines[0]) { const r = parseCSVLine(lines[0]); staff   = (clean(r[1]) || clean(r[0])).trim(); }
      if (lines[1]) { const r = parseCSVLine(lines[1]); manager = (clean(r[1]) || clean(r[0])).trim(); }
      if (!staff && !manager) throw new Error('empty');
      const out = { staff: staff || FALLBACK.staff, manager: manager || FALLBACK.manager };
      localStorage.setItem(PIN_CACHE, JSON.stringify({ ...out, ts: Date.now() }));
      return out;
    } catch (e) {
      clearTimeout(timer);
      // fall back to last good cache if we have one
      try {
        const c = JSON.parse(localStorage.getItem(PIN_CACHE) || 'null');
        if (c && c.staff) return { staff: c.staff, manager: c.manager };
      } catch (_) {}
      return null; // signal "couldn't load and no cache"
    }
  }
  ...
  function buildOverlay(){
    ...
    o.innerHTML = `
      <div class="bla-logo" id="bla-logo">⚡ Banshee Labyrinth</div>
  ...
  function check(){
    ...
    // login mode: manager PIN wins (grants manager), else staff PIN
  ...
  function refreshChip(){
    ...
    chip.className='bla-chip'+(auth.role==='manager'?' mgr':'');
    chip.innerHTML=`<span>Signed in: <b>${auth.role==='manager'?'Manager':'Staff'}</b></span>
  ...
  function isManager(){ const a=getAuth(); return !!a && a.role==='manager'; }
